...
// pdf-parse - use dynamic import since it's an ES module
let pdfParse;
...
async function extractPDFText(pdfPath) {
  ...
  const pdfParseFn = await initPdfParse();
  const data = await pdfParseFn(dataBuffer);
  
  console.log(`✅ Extracted text from PDF (${data.numpages || 'unknown'} pages)`);
  return data.text || '';
}
