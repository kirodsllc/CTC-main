  ...
  try {
    ...
    if (master_part_no) {
      specificFilters.push({
        masterPart: {
          masterPartNo: { contains: master_part_no as string },
        },
      });
    }
    ...
    if (part_no) {
      specificFilters.push({
        partNo: { contains: part_no as string },
      });
    }
    ...
    if (category_id) {
      if (where.AND) {
        where.AND.push({ categoryId: category_id as string });
      } else {
    ...
    if (subcategory_id) {
      ...
      const subcategory = await prisma.subcategory.findFirst({
        where: { name: { contains: subcategory_name as string } },
      });
      if (subcategory) {
        if (where.AND) {
